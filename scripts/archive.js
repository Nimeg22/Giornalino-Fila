...
// const numberOfArticles: dichiarato nel php
// const articlesContent: dichiarato nel php
let tabNum=1
  ...
  tabs.forEach(element => {
    ...
    for(let i = numberOfCells*(tabLooper-2); i < (tabLooper-1)*numberOfCells; i++){
      if (i < numberOfArticles && i >= 0) {
        element.innerHTML+=`
      <a class="article" href="http://localhost/giornalino/html/article-base.php?articleID=${i+1}">
        <img src="${articlesContent[i].thumbnailImgUrl}"> 
        <div class="article-info">
          <label class="article-data">
            ${articlesContent[i].author} &#183; ${articlesContent[i].uploadDate}
          </label>
